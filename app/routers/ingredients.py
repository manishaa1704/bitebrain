...
def create_ingredient(
    ...
    if needs_data:
        ...
        def _fetch_off_data(name: str):
            ...
            headers = {"User-Agent": "BiteBrainAPI/1.0"}
        ...
        try:
            data = _fetch_off_data(ingredient_data.name)

            if data.get('products') and len(data['products']) > 0:
                product = data['products'][0]
                for p in data['products']:
                    n = p.get('nutriments', {})
                    if n.get('energy-kcal_100g') or n.get('energy-kcal') or n.get('energy_100g') or n.get('energy'):
                        product = p
                        break
                ...
                print(f"✅ OpenFoodFacts data fetched for: {ingredient_data.name}")

        except httpx.TimeoutException:
            print(f"⚠️ OpenFoodFacts timed out for: {ingredient_data.name}")
            ...
            print(f"⚠️ OpenFoodFacts lookup failed: {e}")
            for field in ['calories_per_100g', 'protein_per_100g', 'carbs_per_100g', 'fat_per_100g']:
                if ingredient_dict.get(field) is None:
                    ingredient_dict[field] = 0.0
